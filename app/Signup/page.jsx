...
export default function page() {
    ...
    const handleregister = async() => {
        ...
        console.log((inputref.current.children[1].value.split(" ")[1]))
 
        if(!inputref.current.children[1].value){
            return toast.error("Please Enter Your Full Name")
        }   else if (inputref.current.children[1].value.split(" ").length !== 2 || inputref.current.children[1].value.split(" ")[1] == ""){
             return toast.error("Please Enter Full Name With Space")
        }
        ...
        if(!inputref.current.children[2].value){
        ...
        const params = {
            ...
            username:inputref.current.children[1].value,
            password:inputref.current.children[2].value
        }
      ...
      <div className="login w-[100%] h-[100vh] flex items-center justify-center">
        <div className="loginframe flex-col gap-[25px] bg-gray-100 p-[30px] flex items-center justify-center">
            ...
            <div ref={inputref} className="inputs w-[350px] flex flex-col gap-[25px]">
                ...
                <input className='w-[100%] bg-gray-200 p-[10px]' placeholder='Enter Your Full Name' type="text" />

                <input className='w-[100%] bg-gray-200 p-[10px]' placeholder='Enter Your Password' type="password" />

                <button onClick={() => handleregister()} className='bg-blue-500 p-[10px] text-white'>Sign In</button>
            </div>
