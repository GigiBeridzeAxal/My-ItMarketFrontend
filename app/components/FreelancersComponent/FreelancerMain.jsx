...
export default function FreelancerMain() {
    ...
    const {freelancersdata , getfreelancerusers} = useAuth()
  ...
  return (
    <div className="freelancersmain p-[20px]">
        ...
        {freelancersdata.map((data,id) => {
            ...
            return <div className='flex items-start gap-[15px]' key={id}>
                    ...
                    <div className='flex  items-center justify-between'><div className='' >{data.username} | <Link href={`/profile?profileid=${data.id}`} className='font-[400] text-[20px]'>@{data.email.split('@')[0]} </Link> </div> <Link href={`/Message?userid=${data.id}`} className='p-[10px] cursor-pointer bg-blue-500 text-white text-[16px] rounded-[3px]'>Contact</Link></div>
